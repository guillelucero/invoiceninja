        ...
        @if (Auth::user()->canCreateOrEdit(ENTITY_INVOICE, $invoice))
            ...
            @else
                @if (!$invoice->is_deleted)
					@if ($invoice->isSent())
						{!! Button::danger(trans("texts.save_{$entityType}"))->withAttributes(array('id' => 'saveButton', 'onclick' => 'onSaveClick()'))->appendIcon(Icon::create('floppy-disk')) !!}
					@else
						...
						@if (! $invoice->trashed())
							{!! Button::danger(trans($invoice->is_recurring ? "texts.mark_ready" : "texts.mark_sent"))->withAttributes(array('id' => 'saveButton', 'onclick' => 'onMarkSentClick()'))->appendIcon(Icon::create('globe')) !!}
						@endif
					@endif
					@if (! $invoice->trashed())
