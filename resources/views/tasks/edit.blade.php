    ...
    @if (Auth::user()->canCreateOrEdit(ENTITY_TASK, $task))
        @if (Auth::user()->hasFeature(FEATURE_TASKS))
            @if ($task && $task->is_running)
                {!! Button::danger(trans('texts.save'))->large()->appendIcon(Icon::create('floppy-disk'))->withAttributes(['id' => 'save-button']) !!}
            ...
            @else
                ...
                @else
                    {!! Button::danger(trans('texts.save'))->large()->appendIcon(Icon::create('floppy-disk'))->withAttributes(['id' => 'save-button']) !!}
                    {!! Button::danger(trans('texts.start'))->large()->appendIcon(Icon::create('play'))->withAttributes(['id' => 'start-button']) !!}
                @endif
            @endif
        @else
            {!! Button::normal(trans('texts.cancel'))->large()->asLinkTo(HTMLUtils::previousUrl('/tasks'))->appendIcon(Icon::create('remove-circle')) !!}
